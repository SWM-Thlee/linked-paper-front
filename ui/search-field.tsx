...
export const searchVariant = tv({
  slots: {
    ...
    icon: ["text-light-onSurface", "dark:text-dark-onSurface"],
  },
  variants: {
    ui_color: {
      primary: {
...
const SearchField = forwardRef<HTMLInputElement, SearchProps>(
  (
    ...
    const { container, input, icon } = searchVariant({ ui_color, ui_size });
    ...
    return (
      <div key={key} className={container({ className })}>
        <input
          ...
          type={type ?? "text"}
          placeholder={
            placeholder ?? defaultPlaceholder ?? "Search Anything..."
          }
          className={input()}
          {...props}
        />
