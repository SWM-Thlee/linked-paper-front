...
import {
  defaultMobileNodeConfig,
  defaultMobileViewConfig,
} from "@/features/graph/utils/default-mobile-config";
...
export default function FlowerGraphView() {
  ...
  const { nodeConfig } = useGraphNodeConfig({
    initialNodeConfig: defaultMobileNodeConfig,
  });
  const { viewConfig } = useGraphViewConfig({
    initialViewConfig: defaultMobileViewConfig,
  });
  ...
  const { requestFocus, focusOffsetX, focusResolver } = useNodeFocus(
    handler,
    viewConfig,
  );
  ...
  const renderRootNode: Graph.Render.RenderNode = useCallback(
    ({ node, drawCircle, drawText }) => {
      ...
      drawCircle({
        ...
        scale: { start: 0.1, end: 0.2, multiplier: 3 },
      });
